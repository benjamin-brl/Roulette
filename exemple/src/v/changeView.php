    ...
    <div class="content">
        ...
        <form method="POST">
            <table>
                <tr>
                    ...
                    <td class="ldap_input"><input required type="text" name="ldap_host" value="10.0.100.10"></td>
                </tr>
                <tr>
        ...
        if(isset($_POST["ldap_config"])){
            ...
            if(ping($_POST["ldap_host"])){
                ...
                <form method="POST">
                    <label>Choisir une classe a resynchroniser : </label>
                    <select name="classeSelected2">
                        <?php
                            $LdapManager->groups(true);
                        ?>
                    </select>
                    <input type="hidden" name="ldap_host" value=<?=$LdapManager->getLdaphost()?>>
                    <input type="hidden" name="ldap_port" value=<?=$LdapManager->getLdapPort()?>>
                    <input type="hidden" name="ldap_uri" value=<?=$LdapManager->getLdapuri()?>>
                    <input type="hidden" name="ldap_dn" value=<?=$LdapManager->getDn()?>>
                    <input type="submit" value="Valider" name="LDAP2"/>
                </form>
                <?php
            }
            else{
