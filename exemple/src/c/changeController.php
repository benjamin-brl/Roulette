...
if(isset($_POST['LDAP2'])){
	...
	$ldap['manager'] = $manager;
	$LdapManager = new LdapManager($ldap);
	$LdapManager->insertUser($_POST['classeSelected2']);
	echo '<h4 class="valid">La classe LDAP a bien été synchronisée.</h4>';
}
...
if(isset($_POST['upload'])) {																// Verifie si le formulaire est envoyé
	...
	if(move_uploaded_file($tmp_file, $file)) { 													// Verifie si le fichier est bien présent
		...
		foreach($content as $data){
			...
			if($count === 0){
				$classe = $manager->getDb("DISTINCT class");
				foreach($classe as $element){
					if($element['class'] === $donnees['class']){
						$exist = true;
					}
				}
			}
			...
			if($exist){
				$_POST['message'] = "<h4 class='warning'>Upload non effectué, cette classe a déjà été ajouté</h4>";
			}else{
